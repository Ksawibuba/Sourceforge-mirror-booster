...
def get_mirrors(project: str, raw_path: str) -> list[str]:
    ...
    try:
        ...
        console.print("[yellow]⚠  Nie znaleziono mirrorów na stronie – używam pełnej listy zapasowej.[/yellow]")

    except Exception:
        console.print("[yellow]⚠  Błąd pobierania strony (brak dostępu do listy online) – używam pełnej listy zapasowej.[/yellow]")
...
def main():
    ...
    if not url:
        ...
        url = console.input("[bold cyan]🔗 Podaj link do pliku SF:[/bold cyan] ").strip()
    ...
    if not ok_results:
        console.print("[red]❌ Żaden mirror nie odpowiedział poprawnie. Token wygasł lub link jest błędny.[/red]")
        sys.exit(1)
    ...
    console.print(
        Panel(
            ...
            + (f"   |   Latencja: {best['latency_ms']:.0f} ms" if best.get("latency_ms") else "")
    ...
    try:
        choice = console.input(
            "[bold]Pobrać plik teraz? ([cyan]T[/cyan]/n): [/bold]"
        ).strip().lower()
